...
namespace {  // unnamed, autonomous namespace
  ...
  // represented by Epsilon. Usually, this is a pretty small number, but since we are dealing with money for a repair cost
  // (only two, maybe three decimal places) we can be a bit more tolerant. Two floating point values are considered equal if
...
Repair::Repair(const std::string& name, const std::string& vehicle, const std::string& date, const std::string& services, const double& cost)
  ...
  {}  // Avoid setting values in the constructor's body (when possible)
...
std::istream& operator>>(std::istream& stream, Repair& request) {
  ...
  // Assume fields are separated by commas and string attributes are enclosed with double quotes.
  // For example: "John Doe", "Isuzu", "9/29 10:00 am", "synthetic oil change", 91.11
